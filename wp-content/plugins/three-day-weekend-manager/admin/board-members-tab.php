...
function tdwm_board_members_tab() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_role_name'])) {
        ...
        $user_has_role = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $assignments_table WHERE user_id = %d AND end_date IS NULL", $user_id
        ));

        $role_conflict = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $assignments_table WHERE role_name = %s AND end_date IS NULL", $role_name
        ));

        $role_gender_req = $wpdb->get_var($wpdb->prepare(
            "SELECT gender_requirement FROM $roles_table WHERE role_name = %s", $role_name
        ));

        if ($user_has_role) {
            echo '<div class="error"><p>This user is already assigned to an active board role.</p></div>';
        } elseif ($role_gender_req && $role_gender_req !== 'N/A' && strtolower($gender) !== strtolower($role_gender_req)) {
            echo '<div class="error"><p>This role requires a ' . esc_html($role_gender_req) . ' board member. Assignment blocked.</p></div>';
        } elseif ($role_conflict) {
    ...
    foreach ($roles as $role) {
        ...
        if (!empty($assigned)) {
            foreach ($assigned as $entry) {
                ...
                echo '<td><input type="text" name="edit_comment" value="' . esc_attr($entry->comment ?? '') . '" placeholder="Comment"></td>';
    ...
    if ($filter_name) {
        $where[] = "(EXISTS (SELECT 1 FROM {$wpdb->usermeta} m1 WHERE m1.user_id = a.user_id AND m1.meta_key = 'first_name' AND m1.meta_value LIKE %s)
    ...
    $query = "SELECT a.role_name, a.user_id, a.gender, a.start_date, a.end_date, a.comment 
              FROM $assignments_table a 
              WHERE " . implode(' AND ', $where) . " 
              ORDER BY a.end_date DESC";

    $past_members = $wpdb->get_results($wpdb->prepare($query, ...$params));
